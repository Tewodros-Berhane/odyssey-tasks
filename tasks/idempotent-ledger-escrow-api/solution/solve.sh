...
def get_account_balances(conn, account_id: str):
    ...
    if not acc:
        return None
    now_iso = datetime.now(timezone.utc).isoformat()
    row = conn.execute("SELECT COALESCE(SUM(amount), 0) as hold_sum FROM holds WHERE account_id = ? AND status = 'active' AND expires_at > ?", (account_id, now_iso)).fetchone()
    hold_sum = row["hold_sum"] if row else 0
