#include "lockfree_allocator.hpp"
#include <cstdlib>
#include <cstring>
#include <algorithm>

#if defined(_MSC_VER) || defined(_WIN32)
#include <malloc.h>
static inline void* aligned_alloc_16(size_t size) {
    return _aligned_malloc(size, 16);
}
static inline void aligned_free_16(void* ptr) {
    _aligned_free(ptr);
}
#else
static inline void* aligned_alloc_16(size_t size) {
    void* ptr = nullptr;
    if (posix_memalign(&ptr, 16, (size + 15) & ~15) != 0) return nullptr;
    return ptr;
}
static inline void aligned_free_16(void* ptr) {
    std::free(ptr);
}
#endif

namespace lf_alloc {

thread_local ThreadCache Allocator::thread_cache_;

ThreadCache::ThreadCache() {
    for (size_t i = 0; i < NUM_SMALL_CLASSES; ++i) {
        local_bins_[i] = nullptr;
        bin_counts_[i] = 0;
    }
}

ThreadCache::~ThreadCache() {
    for (size_t i = 0; i < NUM_SMALL_CLASSES; ++i) {
        FreeNode* curr = local_bins_[i];
        while (curr) {
            FreeNode* next_node = curr->next.load();
            aligned_free_16(curr);
            curr = next_node;
        }
        local_bins_[i] = nullptr;
        bin_counts_[i] = 0;
    }
}

void* ThreadCache::Allocate(size_t size_class, size_t obj_size) {
    if (size_class < NUM_SMALL_CLASSES && local_bins_[size_class] != nullptr) {
        FreeNode* node = local_bins_[size_class];
        local_bins_[size_class] = node->next.load();
        bin_counts_[size_class]--;
        return static_cast<void*>(node);
    }
    size_t alloc_sz = std::max(obj_size, sizeof(FreeNode));
    alloc_sz = (alloc_sz + 15) & ~15;
    return aligned_alloc_16(alloc_sz);
}

void ThreadCache::Deallocate(size_t size_class, void* ptr) {
    if (!ptr) return;
    if (size_class < NUM_SMALL_CLASSES && bin_counts_[size_class] < 64) {
        FreeNode* node = static_cast<FreeNode*>(ptr);
        node->next.store(local_bins_[size_class]);
        local_bins_[size_class] = node;
        bin_counts_[size_class]++;
    } else {
        aligned_free_16(ptr);
    }
}

size_t Allocator::SizeToClass(size_t bytes) {
    if (bytes <= 16) return 0;
    if (bytes <= 32) return 1;
    if (bytes <= 64) return 2;
    if (bytes <= 128) return 3;
    if (bytes <= 256) return 4;
    if (bytes <= 512) return 5;
    if (bytes <= 1024) return 6;
    return NUM_SMALL_CLASSES - 1;
}

size_t Allocator::ClassToSize(size_t size_class) {
    return (1ULL << (size_class + 4));
}

void* Allocator::Malloc(size_t bytes) {
    if (bytes == 0) return nullptr;
    if (bytes <= MAX_SMALL_SIZE) {
        size_t sc = SizeToClass(bytes);
        return thread_cache_.Allocate(sc, bytes);
    }
    size_t alloc_sz = (bytes + 15) & ~15;
    return aligned_alloc_16(alloc_sz);
}

void Allocator::Free(void* ptr) {
    if (!ptr) return;
    // For small sizes, return to cache or free
    thread_cache_.Deallocate(0, ptr);
}

void* Allocator::Realloc(void* ptr, size_t new_size) {
    if (!ptr) return Malloc(new_size);
    if (new_size == 0) {
        Free(ptr);
        return nullptr;
    }
    void* new_ptr = Malloc(new_size);
    if (!new_ptr) return nullptr;
    std::memcpy(new_ptr, ptr, std::min(new_size, (size_t)32));
    Free(ptr);
    return new_ptr;
}

} // namespace lf_alloc
